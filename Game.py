...
CONFIG.read_file(open("game.conf"))
...
class Game:
    ...
    def __init__(self):
        ...
        self.fog_images = [
            pygame.image.load(os.path.join("images", "fog.png")),  # 0
        ...
        self.wall_images = [
            pygame.image.load(os.path.join("images", "gray.png")),  # 0
        ...
        self.images = [
            pygame.image.load(os.path.join("images", "grass.png")),
        ...
        if self.fullscreen == True:  # check the config file for fullscreen options
            self.screen = pygame.display.set_mode(
                (FULLSCREEN_WIDTH, FULLSCREEN_HEIGHT), FULLSCREEN
            )
        else:
            self.screen = pygame.display.set_mode(
                (self.window_width, self.window_height), RESIZABLE
            )
        ...
        self.stats_offset = (
            math.floor(int(0.8 * self.window_width) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
            TILE_WIDTH,
        )
        self.click_state_offset = (
            math.floor(int(0.8 * self.window_width) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
            TILE_WIDTH * 2,
        )
        self.stat_box_offset = (
            math.floor(int(0.8 * self.window_width) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
            TILE_WIDTH * 5,
        )
        ...
        self.vp_dimensions = (
            math.floor(int(0.8 * self.window_width) / TILE_WIDTH) * TILE_WIDTH,
            math.floor(int(0.8 * self.window_height) / TILE_WIDTH) * TILE_WIDTH,
        )
        self.char_box_top = (
            math.floor(int(0.8 * self.window_height) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH
        )  # rectangle for the char box
        self.char_box_left = 0
        self.char_box_width = (
            math.floor(int(0.8 * self.window_width) / TILE_WIDTH) * TILE_WIDTH
        )
        self.char_box_height = (
            math.floor(int(0.2 * self.window_height) / TILE_WIDTH) * TILE_WIDTH
        )
        self.combat_log_offset = (
            math.floor(int(0.8 * self.window_width) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
            TILE_WIDTH * 20,
        )
        ...
        self.z_up_button_offset = (
            math.floor(int(0.5 * self.window_width) / TILE_WIDTH) * TILE_WIDTH,
            math.floor(int(0.8 * self.window_height) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
        )
        self.z_down_button_offset = (
            math.floor(int(0.6 * self.window_width) / TILE_WIDTH) * TILE_WIDTH,
            math.floor(int(0.8 * self.window_height) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
        )
        self.up_button_offset = (
            math.floor(int(0.4 * self.window_width) / TILE_WIDTH) * TILE_WIDTH,
            TILE_WIDTH / 4,
        )
        self.down_button_offset = (
            math.floor(int(0.4 * self.window_width) / TILE_WIDTH) * TILE_WIDTH,
            math.floor(int(0.8 * self.window_height) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
        )
        self.left_button_offset = (
            TILE_WIDTH / 8,
            math.floor(int(0.4 * self.window_height) / TILE_WIDTH) * TILE_WIDTH,
        )
        self.right_button_offset = (
            self.combat_log_offset[0],
            math.floor(int(0.4 * self.window_height) / TILE_WIDTH) * TILE_WIDTH,
        )
        self.gui_container = gui.Container(align=-1, valign=-1)
        self.gui_container.add(
            self.combat_log, self.combat_log_offset[0], self.combat_log_offset[1]
        )
        self.gui_container.add(
            self.z_up_button, self.z_up_button_offset[0], self.z_up_button_offset[1]
        )
        self.gui_container.add(
            self.z_down_button,
            self.z_down_button_offset[0],
            self.z_down_button_offset[1],
        )
        self.gui_container.add(
            self.up_button, self.up_button_offset[0], self.up_button_offset[1]
        )
        self.gui_container.add(
            self.down_button, self.down_button_offset[0], self.down_button_offset[1]
        )
        self.gui_container.add(
            self.left_button, self.left_button_offset[0], self.left_button_offset[1]
        )
        self.gui_container.add(
            self.right_button, self.right_button_offset[0], self.right_button_offset[1]
        )
        self.gui_container.add(
            self.end_turn_button,
            self.end_turn_button_offset[0],
            self.end_turn_button_offset[1],
        )
        ...
        self.num_x_tiles = int(
            math.ceil(int(self.vp_dimensions[0]) / TILE_WIDTH)
        )  # the number of tiles to be shown at one time for X
        self.num_y_tiles = int(
            math.ceil(int(self.vp_dimensions[1]) / TILE_WIDTH)
        ...
        self.zlevels = 5
        self.mapdata = [
            [[MapTile(1) for cols in range(self.maph)] for rows in range(self.mapw)]
        ...
        self.tiled_bg = pygame.Surface(
            (self.num_x_tiles * TILE_WIDTH, self.num_y_tiles * TILE_WIDTH)
        ).convert()  # IGNORE:E1121
        self.players = []
        self.mobs = []
    ...
    def click_in_viewport(self, x, y):
        ...
        if (
            x < self.num_x_tiles * TILE_WIDTH + self.vp_render_offset[0]
            and x > self.vp_render_offset[0]
            and y < self.num_y_tiles * TILE_WIDTH + self.vp_render_offset[1]
            and y > self.vp_render_offset[1]
            ...
            if player_initiative >= mob_initiative:
                ...
                if player_roll == 1:
                    ...
                    if player_roll == 20:
                        ...
                        damage = (
                            damage * 2
                        )  # crit mutliplier won't be hard coded at some point
                    if m.take_damage(damage):
                        # monster still has hp left after the hit
                        to_log = (
                            m.name + " takes " + str(damage) + " from " + p.name + "!"
                        )
                        self.log.append(to_log)
                        mob_roll = roll_d_20()
                        ...
                        to_log = (
                            m.name
                            + " tries to counter-attack with a roll of "
                            + str(mob_roll)
                            + " +"
                            + str(m.get_level())
                        )
                        ...
                        if mob_roll == 1:
                            ...
                            if mob_roll == 20:
                                # crit
                                to_log = (
                                    m.name + " scored a critical hit on " + p.name + "!"
                                )
                                self.log.append(to_log)
                ...
                if monster_roll == 1:
                    ...
                    monster_chance_to_hit > p.get_defense_bonus() or monster_roll == 20
                ):
                    # hit
                    damage = roll_damage(m.job.damage) + m.get_attack_bonus()
                    if monster_roll == 20:
                    ...
                    if p.take_damage(damage):
                        ...
                        to_log = (
                            p.name + " takes " + str(damage) + " from " + m.name + "!"
                        )
                        self.log.append(to_log)
                        player_roll = roll_d_20()
                        ...
                        if player_roll == 1:
                            ...
                            player_hit_chance > m.get_defense_bonus()
                            or player_roll == 20
                        ):
                            # hit
                            damage = roll_damage(p.job.damage) + p.get_attack_bonus()
                            if player_roll == 20:
                                # crit
                                to_log = (
                                    p.name + " scored a critical hit on " + m.name + "!"
                                )
                                self.log.append(to_log)
                                damage = damage * 2  # crit mult
                            if m.take_damage(damage):
                                ...
                                self.log.append(to_log)
                                to_log = (
                                    p.name + " gained: " + str(m.experience) + " xp!"
                                )
                                self.log.append(to_log)
                                p.gain_xp(m.experience)
                                m.alive = False
    ...
    def draw_players_and_mobs(self):
        ...
        for p in self.players:
            if (p.x, p.y, p.z) in self.view_port:
                self.screen.blit(
                    p.image,
                    self.vp_render_offset,
                    (
                        self.view_port_coord[0] - (p.x * TILE_WIDTH),
                        (self.view_port_coord[1] - (p.y * TILE_WIDTH)),
                    )
                    + self.vp_dimensions,
                )
                self.screen.blit(
                    self.arial_font_bold.render(str(p.hp), True, (255, 0, 0)),
                    (
                        (p.x - self.start_x_tile) * TILE_WIDTH + TILE_WIDTH,
                        (p.y - self.start_y_tile) * TILE_WIDTH + TILE_WIDTH,
                    ),
                )
                if p.selected:
                    green = (0, 255, 0)
                    rect = pygame.Rect(
                        ((p.x * TILE_WIDTH) - self.view_port_coord[0]) + TILE_WIDTH,
                        ((p.y * TILE_WIDTH) - self.view_port_coord[1]) + TILE_WIDTH,
                        TILE_WIDTH,
                        TILE_WIDTH,
                    )
                    pygame.draw.rect(self.screen, green, rect, 3)
        for m in self.mobs:
            if (m.x, m.y, m.z) in self.view_port and self.is_foggy(
                m.x, m.y, m.z
            ) == False:
                self.screen.blit(
                    self.arial_font_bold.render(str(m.hp), True, (255, 0, 0)),
                    (
                        (m.x - self.start_x_tile) * TILE_WIDTH + TILE_WIDTH,
                        (m.y - self.start_y_tile) * TILE_WIDTH + TILE_WIDTH,
                    ),
                )
                self.screen.blit(
                    m.image,
                    self.vp_render_offset,
                    (
                        self.view_port_coord[0] - (m.x * TILE_WIDTH),
                        (self.view_port_coord[1] - (m.y * TILE_WIDTH)),
                    )
                    + self.vp_dimensions,
                )
                if m.selected:
                    red = (255, 0, 0)
                    rect = pygame.Rect(
                        ((m.x * TILE_WIDTH) - self.view_port_coord[0]) + TILE_WIDTH,
                        ((m.y * TILE_WIDTH) - self.view_port_coord[1]) + TILE_WIDTH,
                        TILE_WIDTH,
                        TILE_WIDTH,
                    )
        ...
        for (x, y, z) in self.dead_players:
            if (x, y, z) in self.view_port:
                self.screen.blit(
                    ...
                    self.vp_render_offset,
                    (
                        self.view_port_coord[0] - (x * TILE_WIDTH),
                        (self.view_port_coord[1] - (y * TILE_WIDTH)),
                    )
                    + self.vp_dimensions,
                )
            ...
            for y in range(self.maph):
                if (x, y, self.current_z) in self.view_port:
                    if self.is_foggy(x, y, self.current_z):  # fog
                        ...
                        self.tiled_bg.blit(
                            self.fog_images[value],
                            (
                                (x - self.start_x_tile) * TILE_WIDTH,
                                (y - self.start_y_tile) * TILE_WIDTH,
                            ),
                        )
            ...
            for y in range(self.maph):
                if (x, y, self.current_z) in self.view_port:
                    if self.clickdata[self.current_z][x][y] != 0:
                        self.tiled_bg.blit(
                            self.images[self.clickdata[self.current_z][x][y]],
                            (
                                (x - self.start_x_tile) * TILE_WIDTH,
                                (y - self.start_y_tile) * TILE_WIDTH,
                            ),
        ...
        for p in self.players:
            if p.pathlines:
                for l in p.pathlines:
                    for x in range(
                        int(self.start_x_tile),
                        int(self.start_x_tile + self.num_x_tiles),
                    ):
                        for y in range(
                            int(self.start_y_tile),
                            int(self.start_y_tile + self.num_y_tiles),
                        ):
                            if l[0] == x and l[1] == y and l[2] == self.current_z:
                                ...
                                pygame.draw.rect(
                                    self.tiled_bg, p.color, rect, line_width
                                )
    ...
    def draw_char_box(self):
        ...
        rectangle = pygame.Rect(
            int(self.char_box_left + TILE_WIDTH),
            int(self.char_box_top + TILE_WIDTH),
            int(self.char_box_width),
            int(self.char_box_height),
        )
        gray = (115, 115, 115)
        green = (0, 255, 0)
        red = (255, 0, 0)
        ...
        for p in self.players:
            ...
            p.portrait_rect.left = (
                p.portrait_rect.left + p.portrait_rect.width * count
            )  # (p.portrait_rect.width * count, ry )
            self.screen.blit(p.portrait, p.portrait_rect)
            self.screen.blit(
                self.arial_font.render(str(count + 1), True, white),
                p.portrait_rect.topleft,
            )
            self.screen.blit(
                self.arial_font_bold.render("HP: " + str(p.hp), True, white),
                (p.portrait_rect.left, p.portrait_rect.centery),
            )
            if p.selected == True:
        ...
        for m in self.mobs:
            if m.z == self.current_z:
                if self.is_foggy(m.x, m.y, m.z) == False:
                    ...
                    m.portrait_rect.left = (
                        m.portrait_rect.left + m.portrait_rect.width * count
                    )  # (p.portrait_rect.width * count, ry )
                    self.screen.blit(m.portrait, m.portrait_rect)  #
                    self.screen.blit(
                        self.arial_font_bold.render("HP: " + str(m.hp), True, white),
                        (m.portrait_rect.left, m.portrait_rect.centery),
                    )
                    if m.selected == True:
    ...
    def draw_possible_moves(self):
        ...
        for x in range(
            int(self.start_x_tile), int(self.start_x_tile + self.num_x_tiles)
        ):
            for y in range(
                int(self.start_y_tile), int(self.start_y_tile + self.num_y_tiles)
            ):
            ...
            for dcol in (-1, 0, 1):
                ...
                if (newrow, newcol, z) in self.view_port:
                    if drow == 0 and dcol == 0:
                        ...
                        pass
                    elif self.is_blocked(newrow, newcol, z) or self.is_foggy(
                        newrow, newcol, z
                    ):
                        # print "Blocked or foggy"
                        pass
    ...
    def get_open_spot_around(self, x, y, z):
        ...
        # some bug from porting from python 2 to 3 some ints are now floats, need to track down the culprit
        x = int(x)
        y = int(y)
        z = int(z)
        for drow in (-1, 0, 1):
            ...
            if event.type == QUIT:
                ...
                if self.click_in_viewport(mx, my):
                    ...
                    if self.mouse_box == True and self.click_state == "MoveSelect":
                        self.mouse_box_rect.topleft = (
                            (x - self.start_x_tile) * TILE_WIDTH + TILE_WIDTH,
                            (y - self.start_y_tile) * TILE_WIDTH + TILE_WIDTH,
                        )
                ...
                if 1 in self.buttons:  # left click
                    ...
                    my = self.motion.pos[1]
                    if (
                        self.click_in_viewport(mx, my)
                        and self.click_state == "MoveSelect"
                    ):
                        x, y, z = self.view_port_click_to_coords(mx, my, self.current_z)
    ...
    def handle_keyboard(self, event):
        ...
        # View port Movement
            ...
            if keymods & pygame.KMOD_LSHIFT:
                self.view_port_coord[0] = (
                    self.view_port_coord[0] - self.view_port_shift_step
                )
            else:
            ...
            if keymods & pygame.KMOD_LSHIFT:
                self.view_port_coord[1] = (
                    self.view_port_coord[1] - self.view_port_shift_step
                )
    ...
    def find_moves(self, x, y, z, movement):
        ...
        slist = []
        movement_range = list(range(-movement, movement + 1))
    ...
    def find_fov(self, x, y, z, size):
        ...
        slist = []
        size_range = list(range(-size, size + 1))
                ...
                if item.name == "StairsUp":
                    ...
                    if spot != None:
                        ...
                        if open_spot != None:
                            ...
                            self.log.append(text)
                            p.fov.update(
                                self.find_fov(p.x, p.y, p.z, p.get_view_range())
                            )
                            self.center_vp_on(p.x, p.y, p.z)
                else:
    ...
    def set_fullscreen(self):
        ...
        self.stats_offset = (
            math.floor(int(0.8 * FULLSCREEN_WIDTH) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
            TILE_WIDTH,
        )
        self.click_state_offset = (
        ...
        self.char_box_top = (
            math.floor(int(0.8 * FULLSCREEN_HEIGHT) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH
        )  # rectangle for the char box
        self.char_box_left = 0
        self.combat_log_offset = (
        ...
        self.char_box_height = (
            math.floor(int(0.2 * FULLSCREEN_HEIGHT) / TILE_WIDTH) * TILE_WIDTH
        )
        self.end_turn_button_offset = (
            math.floor(int(0.8 * FULLSCREEN_WIDTH) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
        ...
        self.z_up_button_offset = (
            math.floor(int(0.5 * FULLSCREEN_WIDTH) / TILE_WIDTH) * TILE_WIDTH,
            math.floor(int(0.8 * FULLSCREEN_HEIGHT) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
        )
        self.z_down_button_offset = (
            math.floor(int(0.6 * FULLSCREEN_WIDTH) / TILE_WIDTH) * TILE_WIDTH,
            math.floor(int(0.8 * FULLSCREEN_HEIGHT) / TILE_WIDTH) * TILE_WIDTH
            + TILE_WIDTH,
        )
        self.up_button_offset = (
            math.floor(int(0.4 * FULLSCREEN_WIDTH) / TILE_WIDTH) * TILE_WIDTH,
            TILE_WIDTH / 4,
        )
        self.down_button_offset = (
            math.floor(int(0.4 * FULLSCREEN_WIDTH) / TILE_WIDTH) * TILE_WIDTH,
        ...
        self.left_button_offset = (
            TILE_WIDTH / 8,
            math.floor(int(0.4 * FULLSCREEN_HEIGHT) / TILE_WIDTH) * TILE_WIDTH,
        )
        self.right_button_offset = (
            self.combat_log_offset[0],
            math.floor(int(0.4 * FULLSCREEN_HEIGHT) / TILE_WIDTH) * TILE_WIDTH,
    ...
    def screen_resize(self, w, h):
        ...
        self.stats_offset = (
            math.floor(int(0.8 * w) / TILE_WIDTH) * TILE_WIDTH + TILE_WIDTH,
            TILE_WIDTH,
        )
        self.click_state_offset = (
            math.floor(int(0.8 * w) / TILE_WIDTH) * TILE_WIDTH + TILE_WIDTH,
    ...
    def make_map(self):
        ...
        for z in range(self.zlevels):
            ...
            while num_rooms != max_rooms:
                ...
                x = int(randrange(0, self.mapw - w - 1))
                y = int(randrange(0, self.maph - h - 1))
                ...
                if not failed:
                    ...
                    if num_rooms == 0:
                        if z != 0:
                            ...
                            self.mapdata[z][new_x + 1][new_y - 1].content.append(
                                stairs_down
                            )
                        if starting_floor:
                        ...
                        z != max(range(self.zlevels))
                        and num_rooms >= (max_rooms - 2)
                        and upstairs_flag == False
                    ):
                        upstairs_flag = True
                        ...
                        if roll_d_10() > 3:
                            ...
                            for m in self.mobs:
                                m.fov.update(
                                    self.find_fov(m.x, m.y, m.z, m.get_view_range())
                                )
                        else:
                            IG = ItemGenerator()
    ...
    def view_port_click_to_coords(self, x, y, z):
        ...
        return (
            (
                int(
                    (x - self.vp_render_offset[0] + self.view_port_coord[0])
                    / TILE_WIDTH
                )
            ),
            int(
                ((y - self.vp_render_offset[1] + self.view_port_coord[1]) / TILE_WIDTH)
            ),
            z,
        )
    ...
    def render(self):
        ...
        self.screen.blit(
            ...
            self.vp_render_offset,
            (
                self.view_port_coord[0] - (self.start_x_tile * TILE_WIDTH),
                (self.view_port_coord[1] - (self.start_y_tile * TILE_WIDTH)),
            )
            + self.vp_dimensions,
        )
        self.draw_mouse_box()
        ...
        self.screen.blit(
            self.arial_font.render(
                "coordinates: "
                + str(self.view_port_coord[0] / TILE_WIDTH)
                + ", "
                + str(self.view_port_coord[1] / TILE_WIDTH)
                + " Z: "
                + str(self.current_z),
                True,
                (255, 255, 255),
        ...
        self.screen.blit(
            self.arial_font.render(
                "State: " + str(self.click_state), True, (255, 255, 255)
            ),
            self.click_state_offset,
        )
        self.draw_char_box()
        ...
        if self.win:
            self.screen.blit(
                self.arial_font.render("You Win!", True, (255, 255, 255)),
                (self.window_width / 2, self.window_height / 2),
            )
        elif self.win == False:
            self.screen.blit(
                self.arial_font.render("You Lose!", True, (255, 255, 255)),
                (self.window_width / 2, self.window_height / 2),
            )
